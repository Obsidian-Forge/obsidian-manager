...
export class ManagerModal extends Modal {
	...
	public async showHead() {
		...
		// [搜索行]
		const searchBar = new Setting(this.titleEl)
			.setClass("manager-bar__search")
			.setName(this.manager.translator.t("通用_搜索_文本"));

		// [搜索行] 未分组
		const noGroupBar = new ButtonComponent(searchBar.controlEl).setIcon(
			"group"
		);
		...
		this.onlyEnabled
			? onlyEnabled.setIcon("toggle-right")
			: onlyEnabled.setIcon("toggle-left");
		...
		const tagCounts: { [key: string]: number } =
			this.settings.Plugins.reduce((acc, plugin) => {
	...
	public async showData() {
		const plugins: PluginManifest[] = Object.values(
		...
		for (const plugin of plugins) {
			...
			// 插件是否开启
			const isEnabled = this.settings.DELAY
				? ManagerPlugin?.enabled
				: this.appPlugins.enabledPlugins.has(plugin.id);
			if (ManagerPlugin) {
				// [搜索] 仅启用
				...
				if (this.settings.PERSISTENCE) {
					...
					if (
						...
						ManagerPlugin.group !== this.settings.FILTER_GROUP
					)
						continue;
					// [搜索] 标签
					if (
					...
					if (
						this.settings.FILTER_DELAY !== "" &&
						ManagerPlugin.delay !== this.settings.FILTER_DELAY
					)
						continue;
				} else {
					// [搜索] 分组
					...
					if (
						this.tag !== "" &&
						!ManagerPlugin.tags.includes(this.tag)
					)
						continue;
					// [搜索] 标签
				...
				if (!this.editorMode) {
					// [按钮] 打开设置
					if (isEnabled) {
						const openPluginSetting = new ExtraButtonComponent(
							itemEl.controlEl
						);
					...
					// [按钮] 删除插件
					const deletePluginButton = new ExtraButtonComponent(
						itemEl.controlEl
					);
	...
	public count(): string {
		...
		if (this.settings.DELAY) {
			...
			totalCount =
				Object.keys(this.manager.appPlugins.manifests).length - 1;
